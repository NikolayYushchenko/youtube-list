...
class ParseURL:
    ...
    def login(self):
        ...
        for category_url in Settings.category_url:
            if self.check_authorization():
                ...
                self.check_update_cart(carts_url)
    ...
    def check_start_page(self, category_url):
        ...
        if Settings.start_page > 0:
            ...
            return start_url
    def check_update_cart(self, carts_url):
        # //app-product-single//dd/del[1]  ---MBR(normal price)
        # //app-product-single//dd/p[3] ---price
        # //app-product-single//dd/p[3]/span ---discount

        id_old = open_exel.get_column_values_by_name("uma_data.xlsx","ID")
        normal_price_old = open_exel.get_column_values_by_name("uma_data.xlsx","Normal price(USD)")
        price_old = open_exel.get_column_values_by_name("uma_data.xlsx","Price")
        discount_old = open_exel.get_column_values_by_name("uma_data.xlsx","Discount%")

        for index, data  in enumerate(carts_url['id']):
            id_new = str(re.search(r'https://umma\.io/product/(\d+)', data).group(1))
            if(id_new not in id_old):
                print(f"New cart:{data}")
            if(id_new in id_old):
                get_index = id_old.index(id_new)
                print(f"Old cart: {id_new}---{id_old[get_index]}")
                #print(normal_price_old[get_index].strip())
                if ((normal_price_old[get_index].strip() in carts_url['normal_price'][index].strip()) and
                    (price_old[get_index].strip() in carts_url['price'][index].strip()) and
                    (discount_old[get_index].strip() in carts_url['discount'][index].strip())):
                    print(f"Ціна без знижки не змінена: {normal_price_old[get_index]}--{carts_url['normal_price'][index]}")
                    print(f"Ціна зі знижкою не змінена: {price_old[get_index]}--{carts_url['price'][index]}")
                    print(f"Знижка не змінена: {discount_old[get_index]}--{carts_url['discount'][index]}")

                else:
                    print("Ціни і знижка змінені, потрібно оновити дані")
                    print(f"Ціна без знижки  змінена!!!  {normal_price_old[get_index]}--{carts_url['normal_price'][index]}")
                    print(f"Ціна зі знижкою змінена!!! {price_old[get_index]}--{carts_url['price'][index]}")
                    print(f"Знижка змінена!!!  {discount_old[get_index]}--{carts_url['discount'][index]}")

    def parse_container_category(self, category_url, category_parse_step, category_items={}):
        def check_paginator(pagin_code, category_parse_step, category_items_link, current_step=0):
            ...
            if next_sibling and "next" not in next_sibling.get("class", []):
                if category_parse_step > current_step or category_parse_step == 0:
                    ...
                    self.parse_container_category(self.driver.current_url, category_parse_step, category_items)
            else:
        ...
        normal_price_elements = self.driver.find_elements(By.XPATH, '//app-product-single//dd/del[1]')
        price_elements = self.driver.find_elements(By.XPATH, '//app-product-single//dd/p[3]')
        discount_elements = self.driver.find_elements(By.XPATH, '//app-product-single//dd/p[3]/span')

        if not os.path.exists("category"):
            os.mkdir("category")
        if not os.path.exists("category/" + product_elements[0].get_attribute('data-list')):
            os.mkdir(f"category/{product_elements[0].get_attribute('data-list')}")

        if 'id' not in category_items:
            category_items.setdefault('id', [])
        for item in product_elements:
            category_items['id'].append(f"https://umma.io/product/{item.get_attribute('data-id')}")

        if 'normal_price' not in category_items:
            category_items.setdefault('normal_price', [])
        for item in normal_price_elements:
            item = re.search(r'\d+\.\d+', item.text).group()
            category_items['normal_price'].append(f"{item}")

        if 'price' not in category_items:
            category_items.setdefault('price', [])
        for item in price_elements:
            item = self.driver.execute_script('return arguments[0].firstChild.textContent;', item)
            item = re.search(r'\d+\.\d+', item).group()
            category_items['price'].append(f"{item}")

        if 'discount' not in category_items:
            category_items.setdefault('discount', [])
        for item in discount_elements:
            item = re.search(r'\d+', item.text).group()
            category_items['discount'].append(f"{item}")

        check_paginator(paginator, category_parse_step, category_items)
